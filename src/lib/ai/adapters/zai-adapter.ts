...
export function logZAIUsage(label: string, usage: any, modelName: string, pricing?: ModelPricing) {
  if (!pricing) return;
...
export class ZAIProvider implements AIProvider {
  ...
  constructor(apiKey: string, modelName: string, displayName?: string, pricing?: ModelPricing, compactAt?: number) {
    ...
    this.name = displayName || "GLM-5 Turbo";
  ...
  async generateStream(
    ...
    try {
      await runToolLoop({
        ...
        logUsage: (usage) => logZAIUsage('ZAI', usage, this.modelName, this.pricing),
      });
    } catch (error: any) {
      if (error.name === 'AbortError') {
        console.log('ZAI stream aborted by signal');
        return;
      }
      ...
      throw new Error(`ZAI generation failed: ${error.message || 'Unknown error'}`);
    }
  }
